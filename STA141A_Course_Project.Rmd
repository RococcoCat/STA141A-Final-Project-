...
The purpose of Dimension Reduction is to find any useful patterns in the data or to simply our models by filtering out useful predictor variables. I will be using t-SNE to visualize patterns and PCA to find informative predictor variables. 
...
log_model.benchmark <- glm(feedback ~ .,data = benchmark.train, family = "binomial")
predictions_log.bench <- predict(log_model.benchmark, benchmark.test, type="response")
predicted_feedback_log_bench <- (predictions_log.bench>0.5)*1
summary(log_model.benchmark)

cm.log.bench = confusionMatrix(factor(benchmark.test$feedback),factor(predicted_feedback_log_bench), mode = "everything")
print(cm.log.bench)

# ROC
pred <- prediction(predictions_log.bench, benchmark.test$feedback)
perf <- performance(pred, "tpr", "fpr")
plot(perf)
abline(0, 1, col="red")
auc(benchmark.test$feedback, predictions_log.bench, main = "ROC Curve")
...
The random forest model does not have underlying assumptions about the data and therefore have increased flexibility and possibly accuracy compared to regression models (logistic regression). This model is also an improvement upon decision trees because over-fitting is not as much of a concern. Random forest models are also simpler to interpret compared to other non parametric models as it is essentially an amalgamation of decision trees which explain the decisions it makes based on each node. This allows us to generate a feature importance plot which tells us the importance of each feature for predictions. 
...
The variable importance plot shows the importance of each variable in classifying the data. The mean decrease in accuracy plot shows how much the accuracy of the model will decrease if that predictor variable was excluded from the model. The mean decrease in Gini coefficient shows how each variable contributes to the homogeneity of the nodes and leaves in the model. Higher vales in the mean decrease of Gini means that the variable produces more discriminative nodes and higher values in both graphs correlates with greater importance of that predictor variable. As we can see in the graphs, contrast, root and CA3 are very important in producing the random forest model. [28]
...
# mouse.train$feedback = factor(mouse.train$feedback, levels = c(0, 1))
...
acc[nrow(acc), ] = get_perf_metrics(benchmark.test, predictions_log.bench, cm.log.bench, "Benchmark (Logistic regression)")
...
Though the benchmark model has a high accuracy and balanced accuracy, its F1 score is quite low which could mean that it would not perform as well on a dataset dataset (equal distribution of classes). The best model appears to be the **Random Forest** as it has the highest accuracy, balanced accuracy, and AUC and has a relatively high F1 score compared to the other models. 
...
Since the Benchmark model has a high AUC but a low F1-score, this could mean that the model may perform well on the skewed dataset but given a balanced dataset, the performance may decrease. AUC measures model performance based on false positive vs true positive while F1-score measures precision vs recall. The F1 score is computed at a particular decision threshold which I have chosen to be 0.5 while the AUC is computed across all thresholds so it could be possible that the benchmark is not performing well because I have not chosen an appropriate threshold. 
